...
import { submitEventRegistration } from "@/lib/queries/submissions";

type Props = { eventId: string };

export default function EventRegisterForm({ eventId }: Props) {
  ...
  async function handleSubmit() {
    ...
    setStatus("loading");
    const { error } = await submitEventRegistration({ event_id: eventId, ...form });
  ...
  return (
    <div className="bg-[#202151] p-8 md:p-10 flex flex-col gap-6" dir="rtl">
      ...
      <button
        ...
        className="w-full text-[#202151] bg-[#D0B66A] text-[14px] font-black py-4 rounded-sm tracking-wide transition-opacity duration-200 hover:opacity-85 disabled:opacity-40 disabled:cursor-not-allowed"
        style={{ fontFamily: "'Noto Kufi Arabic', sans-serif" }}
      >
